...
const SignUp = () => {
	...
	const onChangeNickName = useCallback(
		(e: React.ChangeEvent<HTMLInputElement>) => {
			...
			} else if (blank_pattern2.test(inputNickName)) {
				setNickNameMessage("공백이 입력되었습니다.");
			} else if (special_pattern.test(inputNickName)) {
				setNickNameMessage("특수문자가 입력되었습니다.");
			} else {
				setNickNameMessage("중복확인 해주세요");
			}
		},
		[],
	);
	...
	const onChangeEmail = useCallback(
		(e: React.ChangeEvent<HTMLInputElement>) => {
			...
			setEmail(inputEmail);
			if (inputEmail.length < 10 || inputEmail.length > 30) {
				setEmailMessage("올바른 이메일 형식이 아닙니다.");
				setIsEmailAvailable(false);
			} else if (inputEmail.replace(blank_pattern, "") === "") {
				setEmailMessage("공백만 입력되었습니다.");
				setIsEmailAvailable(false);
			} else if (blank_pattern2.test(inputEmail)) {
						...
						<tbody>
							...
							<tr className="signup-name">
								...
								<td>
									...
									<div
										className={`message ${
											!isNameValid ? "error" : name.length > 0 ? "success" : ""
										}`}
									>
										{nameMessage}
									</div>
							...
							<tr>
								...
								<td>
									...
									<div
										className={`message ${
											!isAge ? "error" : age.length < 8 ? "success" : ""
										}`}
									>
										{ageMessage}
									</div>
							...
							<tr>
								...
								<td>
									...
									<div
										className={`message ${
											!isPhone ? "error" : phone.length > 0 ? "success" : ""
										}`}
									>
										{phoneMessage}
									</div>
							...
							<tr>
								...
								<td>
									<input
										...
										value={zipCode}
										readOnly
										className="category-address"
										placeholder="우편번호"
									></input>
									<button className="search-btn" onClick={modalOpen}>
										검색
									</button>
									<br />
									<input
							...
							<tr>
								...
								<td>
									<input
										...
										placeholder="비밀번호를 입력해 주세요"
										className={`category-box ${
											!isPasswordConfirm && "invalid-input"
										}`}
							...
							<tr>
								...
								<td className="signup-td">
									<input
										...
										className={`category-box ${
											!isNicknameAvailable && "invalid-input"
										}`}
										name="nickname"
										title="nickName"
